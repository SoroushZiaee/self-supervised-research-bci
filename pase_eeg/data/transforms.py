...
class PSD(object):
    def __init__(
        ...
        if self.stats_name is not None:
            ...
            self.stats = apply_to_collection(
                self.stats,
                dtype=Tensor,
                function=lambda a: a.numpy(),
            )
        else:
            self.stats = None
    ...
    def __call__(
        ...
        if self.stats is not None:
            psd_data = dict(
                (k, ((v - self.stats[k]["mean"]) / self.stats[k]["std"])[0:123])
                for k, v in psd_data.items()
            )
        label[self.name] = psd_data
...
class Kurtosis(object):
    ...
    def __call__(
        ...
        label[self.name] = apply_to_collection(
            data,
            dtype=np.ndarray,
            function=mnf.univariate.compute_kurtosis,
        )
...
class PTPAmplitude(object):
    def __init__(self, name="ptp-amp"):
        self.name = name
    ...
    def __call__(
        ...
        label[self.name] = apply_to_collection(
            data,
            dtype=np.ndarray,
            function=mnf.univariate.compute_ptp_amp,
        )
...
class Skewness(object):
    ...
    def __call__(
        ...
        label[self.name] = apply_to_collection(
            data,
            dtype=np.ndarray,
            function=mnf.univariate.compute_skewness,
        )
