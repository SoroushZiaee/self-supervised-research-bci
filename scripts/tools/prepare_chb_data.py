...
def create_dataframe_from_data(
    chb_mit_root: str,
    patients: list = [1, 3, 5, 6, 7, 8, 10, 23],
):
    ## Extract the All patient data

    for i, patient in tq(enumerate(patients), desc="Extract the data"):
        if not i:
            df = Patient(chb_mit_root, patient, verbose=False).generate_metadata()
...
def chunk_data(df: pd.DataFrame, clip_path: str):
    ## Save the clip of data
    ...
    for _, row in df.iterrows():
        ...
        patient = CbhEdfFileMNE(row["filename"], verbose=False)
        ...
        fs = int(patient.get_sampling_rate())

        for idx, i in enumerate(range(row["start"], row["end"] - fs, fs)):
            data = patient.get_data(start=i, end=i + fs)
            path = os.path.join(clip_path, base_filename + f"_trial_{idx}")
            patient.save_data(path, data)

            ## Store metadata
            output["patient"].append(patient_name)
            output["filename"].append(path)
            output["label"].append(label)

        logging.info(
            f"{base_filename = }, {patient.get_file_duration()}, {patient_name = }"
        )

    output = pd.DataFrame(output)
    output.to_csv(os.path.join(clip_path, "meta_data.csv"))


def run():
    # Define the root of data
    CHB_MIT_ROOT = "/mnt/data/chb_mit"
    CHUNK_DST = os.path.join(CHB_MIT_ROOT, "npy_files")

    logging.info("Preparing the dataset")
    df = create_dataframe_from_data(CHB_MIT_ROOT)
    chunk_data(df, CHUNK_DST)
